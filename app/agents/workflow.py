...
class JobManager:
    ...
    def auto_process_job(self, job_id: str, segment_seconds: int = 900) -> None:
        """飞书Bot全自动处理：detect → plan → render全部段（无人工审核）。

        在后台线程中执行，通过 state["status"] 跟踪进度。
        """
        import threading as _t
        thread = _t.Thread(target=self._auto_process_in_background,
                           args=(job_id, segment_seconds), daemon=True)
        thread.start()

    def _auto_process_in_background(self, job_id: str, segment_seconds: int) -> None:
        """后台线程：全自动处理视频。"""
        try:
            state = self.jobs[job_id]
            with self._lock:
                state["status"] = "detecting"
                state["render_progress"] = {"current_seg": 0, "total_segs": 0,
                                            "status": "detecting", "message": "正在检测画面..."}

            # 检测
            state = nodes.detect_overlay(state)

            # 分割
            with self._lock:
                state["status"] = "rendering"
            state = nodes.plan_segments(state)
            segs = state["segments"]
            total = len(segs)

            with self._lock:
                state["render_progress"] = {"current_seg": 0, "total_segs": total,
                                            "status": "rendering", "message": f"开始渲染 {total} 段..."}

            # 逐段渲染
            for i, seg in enumerate(segs):
                with self._lock:
                    state["current_segment_idx"] = i
                    state["render_progress"] = {"current_seg": i, "total_segs": total,
                                                "status": "rendering",
                                                "message": f"正在渲染第 {i+1}/{total} 段..."}
                seg["final_end_sec"] = float(seg["end_sec"])
                try:
                    nodes.render_segment(state)
                except Exception as e:
                    logger.exception("[%s] 自动渲染 seg%d 失败: %s", job_id, i, e)
                    seg["status"] = "failed"
                    seg["error"] = str(e)

            with self._lock:
                state["status"] = "completed"
                state["render_progress"] = {"current_seg": total - 1, "total_segs": total,
                                            "status": "done", "message": f"全部完成，共 {len(state['final_outputs'])} 段成品"}
            logger.info("[%s] 自动处理完成: %d 段成品", job_id, len(state["final_outputs"]))

        except Exception as e:
            logger.exception("[%s] 自动处理失败: %s", job_id, e)
            with self._lock:
